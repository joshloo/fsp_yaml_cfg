...
class DscToYaml ():
    def __init__ (self):
        ...
        self.Prefix     = ''
        self.TempName   = {'CFGHDR' : 'CfgHeader'}
    ...
    def FormatTemplateName (self, text, parts):
        if text in self.TempName:
            return self.TempName[text]

        # Rename name for template instances
        if text in ['SPD']:
            idx = 3
        elif text in ['GPIO', 'BOOT_OPTION', 'PCIE_PER_CLK', 'CIERP_PERPORT']:
            idx = 2
        else:
            idx = 0
        if idx > 0:
            text = text + '_' + ''.join(parts[1:idx])
        return text
        ...
        if Match.group(1) == '!BSF':
            ...
            for Each in Result:
                ...
                if Key == 'field':
                    ...
                    ConfigDict.clear()
                    ConfigDict['cname']  = self.FormatTemplateName (TmpName, Parts)
    ...
    def ProcessDscLines (self, Lines):
        ...
        for Line in Lines:
            ...
            if Ret:
                ...
                if Cfgs and Cfgs[-1]['cname'] == 'CfgHeader' and ConfigDict['cname'][0] == '<':
    ...
    def FormatName (self, Name):
        NewName = re.sub(r'(_\$\(\d+\))$', '', Name)
        if NewName:
            Name = NewName

        NewName = re.sub(r'(\$\(\d+\))', '',  Name)
        if NewName:
            Name = NewName
    ...
    def TemplateFixup (self, TmpName, TmpList):
        ...
        elif TmpName in ['BOOT_OPTION_TMPL']:
            for Each in TmpList:
                if Each['cname'] == 'CfgHeader':
                    Each['expand'] = Each['expand'].replace('BOOT_OPTION_CFG_DATA_$(1),', 'BOOT_OPTION_CFG_DATA,')

    def ConfigFixup (self, CfgList):
        for Cfg in CfgList:

            if Cfg['cname'] == 'GpioHeaderSize':
                if Cfg['value'] == '_LENGTH_GPIO_CFG_HDR_':
                    Cfg['value'] = '_LENGTH_GPIO_CFG_HDR_ - 8'

            # For QEMU
            if Cfg['cname'] in ['GpioItemSize', 'GpioItemCount']:
                Cfg['value'] = Cfg['value'].replace('_OFFSET_GPIO_DATA_GPP_', '_OFFSET_GPIO_GPP_')
    ...
    def OutputDict (self, Cfgs):
        ...
        for Each in Cfgs:
            ...
            if Prefix != '>':
                Lines.append ('%s- %-12s : %s' % (Padding , self.FormatName (Name), Each.get('expand', '')))
            for Field in Each:
...
def main():
    ...
    if True:
        ...
        fo = open('%s_tmp%s' % (parts[0], parts[1]), 'w')
        for line in lines:
            fo.write(line + '\n')
        fo.close()
    ...
    if True:
        ...
        fo = open('%s_opt%s' % (parts[0], parts[1]), 'w')
        for line in lines:
            #print (line)
            fo.write(line + '\n')
        fo.close()
